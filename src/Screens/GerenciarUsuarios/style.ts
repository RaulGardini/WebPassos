...
export const BackButton = styled.button<{ active?: boolean }>`
  background-color: ${props => props.active ? '#494444ff' : '#211F1F'};
  color: white;
  padding: 0.5rem 1.5rem;
  border: none;
  border-radius: 4px;
...
export const Message = styled.div<{ tipo: string }>`
    padding: 12px 16px;
    border-radius: 6px;
    margin-bottom: 20px;
    font-weight: 500;
    background: ${props => props.tipo === 'success' ? '#d4edda' : '#f8d7da'};
    color: ${props => props.tipo === 'success' ? '#155724' : '#721c24'};
    border: 1px solid ${props => props.tipo === 'success' ? '#c3e6cb' : '#f5c6cb'};
`;
...
export const TipoBadge = styled.span<{ tipo: 'Professor' | 'Admin' }>`
    padding: 4px 8px;
    border-radius: 12px;
    font-size: 0.85rem;
    font-weight: 500;
    background: ${props => props.tipo === 'Admin' ? '#dc3545' : '#28a745'};
    color: white;
`;